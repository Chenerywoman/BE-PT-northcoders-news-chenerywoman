...
exports.findAllArticles = () => Article.find().lean().select('-__v').populate('belongs_to', ['_id', 'title']).populate('created_by', ['_id', 'username', 'avatar_url']).lean();

exports.findArticleById = (id) => Article.findById(id).select('-__v').populate('belongs_to', ['_id', 'title']).populate('created_by', ['_id', 'username', 'avatar_url']).lean();
